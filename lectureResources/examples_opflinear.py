...
#input
buses = pd.read_excel('.\\Inputs\\powerSystem.xlsx', sheet_name='bus')
generations = pd.read_excel('.\\Inputs\\powerSystem.xlsx', sheet_name='generation')
loads = pd.read_excel('.\\Inputs\\powerSystem.xlsx', sheet_name='load')
lines = pd.read_excel('.\\Inputs\\powerSystem.xlsx', sheet_name='line')
Nb = len(buses)
Ng = len(generations)
Nd = len(loads)
Nl = len(lines)
...
for var in [Pg, Pl, theta]:
    for j in range(len(var)):
        print('{}: {}'.format(var[j], pyo.value(var[j])))